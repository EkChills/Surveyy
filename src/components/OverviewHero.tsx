...
export async function OverviewHero({surveyId}:{surveyId:string}) {
    ...
    return (
        ...
        <span className={cn('rounded-lg border flex items-center max-w-full sm:max-w-[26rem] ', buttonVariants({variant:'outline'}))}><span className='text-sm md:text-base font-semibold text-[#1B324D] truncate'>{copyUrl}</span><CopyLink copyText={copyUrl} /></span>
        </>
